...
test('AI makes one bounded drafting request, exposes no tools and persists through the outbox',async()=>{
 let requests=0;const completed=[];
 const db={call:async(name,...args)=>{
  if(name==='job_context')return {settings:{enabled:true,instructions:'Helpful'},thread:{generation:7},contact:{},business:{name:'General business',profile:{}},history:[{direction:'inbound',body:'Hello'}]};
  completed.push([name,...args]);return {jobId:'sms-job'};
 }};
 const response=await processAi({id:'ai-job',lease_token:'token',payload:{generation:7}},db,{apiKey:'test',fetchImpl:async(url,options)=>{
  requests++;const body=JSON.parse(options.body);assert.equal(body.tools,undefined);assert.equal(body.store,false);assert.ok(options.signal);
  return Response.json({status:'completed',output:[{type:'message',content:[{type:'output_text',text:'How can we help?'}]}]});
 }});
 assert.equal(requests,1);assert.equal(response.jobId,'sms-job');assert.equal(completed[0][0],'complete_ai');
});
test('AI drafting supplies a valid input when a conversation has no history',async()=>{
 const db={call:async name=>name==='job_context'?{settings:{enabled:true},thread:{generation:0},contact:{},business:{name:'Test'},history:[]}:null};
 await processAi({id:'job',lease_token:'token',payload:{generation:0}},db,{apiKey:'test',fetchImpl:async(url,options)=>{
  assert.ok(JSON.parse(options.body).input[0].content.length>0);
  return Response.json({status:'completed',output:[{type:'message',content:[{type:'output_text',text:'Test message.'}]}]});
 }});
});
