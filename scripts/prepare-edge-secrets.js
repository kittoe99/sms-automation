...
for(const name of ['SMS_WORKER_SECRET','AUTOMATION_WORKER_SECRET','AI_WORKER_SECRET','PROVISIONING_WORKER_SECRET']) result[name] ||= crypto.randomBytes(32).toString('hex');
for(const name of ['CLERK_ISSUER','CLERK_PUBLISHABLE_KEY','CRM_ALLOWED_ORIGINS','OPENAI_API_KEY']) if(local[name]) result[name]=local[name];
