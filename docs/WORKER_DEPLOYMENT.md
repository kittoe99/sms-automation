...
- `ai-worker`: one drafting request, 40-second timeout, no tools or agent loop; saves the reply through the SMS outbox after checking its conversation generation.
...
AI additionally requires `OPENAI_API_KEY`; its configured model is `gpt-5.4-mini-2026-03-17`. AI stays disabled while that key is missing. Provisioning uses the existing Twilio master credential only in the provisioning handler. Business SMS submissions retrieve tenant-specific credentials from Vault and never inherit the master sender.
...
3. Deploy all seven functions with the repository import map; configure secrets, Clerk and frontend origins.
